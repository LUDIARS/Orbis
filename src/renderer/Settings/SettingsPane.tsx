...
export function SettingsPane({ open, onClose }: { open: boolean; onClose: () => void }): ReactElement | null {
  ...
  const reportError = (reason: unknown): void => setError(reason instanceof Error ? reason.message : 'Unable to update settings.')
  /** @implements SPEC-ORBIS-P3-SETTINGS */
  ...
  const updateVitrum = async (spec: VitrumViewState['spec']): Promise<void> => {
    setError(null)
    try { setVitrum(await window.orbis.setVitrum(spec)) } catch (reason) { reportError(reason) }
  }
  ...
  return <aside style={{ position: 'fixed', zIndex: 50, right: 8, top: 8, width: 390, boxSizing: 'border-box', maxHeight: '90vh', overflow: 'auto', background: '#fff', color: '#111', padding: 14, boxShadow: '0 2px 16px #0008' }}>
    ...
    <h3>Display filter</h3><select aria-label="Display filter" value={vitrum?.spec.id ?? 'none'} onChange={(event) => { void updateVitrum({ id: event.target.value, filters: [] }) }}>{customVitrumId && <option value={customVitrumId}>{customVitrumId}</option>}{(vitrum?.presets ?? ['none']).map((preset) => <option key={preset}>{preset}</option>)}</select>
    <h4>Custom parameter</h4><select aria-label="Filter kind" value={filterKind} onChange={(event) => setFilterKind(event.target.value as typeof filterKind)}>{['brightness', 'contrast', 'saturate', 'hue-rotate', 'invert', 'sepia', 'grayscale', 'blur'].map((kind) => <option key={kind}>{kind}</option>)}</select><input aria-label="Filter value" type="number" value={filterValue} onChange={(event) => setFilterValue(event.target.value)} /><button onClick={() => { const value = Number(filterValue); if (filterValue.trim() === '' || !Number.isFinite(value)) { reportError(new Error('Filter value must be a number.')); return } void updateVitrum({ id: 'custom', filters: [{ kind: filterKind, value }] }) }}>Apply custom filter</button>
  </aside>
}
