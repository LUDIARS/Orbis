...
- 状態: 項目 1〜3・5 = 調査報告 (実装済みの確認)、項目 4 = 設計 (提案)、項目 6 = 調査提案。項目 4・6 の採否は neco 判断
...
## 4. 最終レンダリング結果のシェーダ調整 — 設計 (新モジュール **Vitrum**)
...
**推奨: Tier 1 を次の後続フェーズ候補として実装し、Tier 2 は技術検証と対象ページ種別の需要確認後に
別フェーズで追加。フェーズ番号は正本設計書へ追補するときに確定する。**
...
1. 項目 4: Vitrum Tier 1 先行の採否 (推奨: 採用。フェーズ番号は正本追補時に確定)
2. 項目 5: ジェスチャ拡張 3 点の採否 (推奨: ホイールトリガ + 適用面拡大)
3. 項目 6: Step1/Step2 の採否と、Step3 (Rive) のアセット制作体制
4. 項目 2: Concordia 側 sigillum バインドと Orbis 側 `service_detail` 検証の着手時期
